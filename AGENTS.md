...
## Keywords
TTS, text-to-speech, Qwen3, Apple Silicon, MLX, PyTorch, FastAPI, Telegram bot, CLI, voice synthesis, voice cloning, voice design, audio generation, model inference

## Annotation
Local text-to-speech stack built around Qwen3 TTS models with three transport adapters (HTTP server, Telegram bot, interactive CLI) and a shared runtime core providing model registry, backend selection, job orchestration, and observability.
...
log_event(
    LOGGER,
    level=20,
    event="[ModuleName][functionName][BLOCK_NAME]",
...
core/
  ... shared runtime with GRACE markup ...
server/
  ... HTTP adapter with GRACE markup ...
telegram_bot/
  ... Telegram adapter with GRACE markup ...
cli/
  ... CLI adapter with GRACE markup ...
tests/
