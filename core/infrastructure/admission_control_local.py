...
from core.application.admission_control import QuotaDecision, QuotaGuard, RateLimitDecision, RateLimiter
...
class DisabledRateLimiter(RateLimiter):
    def check_and_consume(self, *, principal_id: str, policy: str) -> RateLimitDecision:
        return RateLimitDecision(allowed=True, policy=policy, limit=0, window_seconds=60, retry_after_seconds=None)


class DisabledQuotaGuard(QuotaGuard):
    def check_and_consume_compute(self, *, principal_id: str) -> QuotaDecision:
        return QuotaDecision(allowed=True, policy="compute_requests", limit=0, window_seconds=None, retry_after_seconds=None)

    def check_active_async_jobs(self, *, principal_id: str) -> QuotaDecision:
        return QuotaDecision(allowed=True, policy="active_async_jobs", limit=0, window_seconds=None, retry_after_seconds=None)
...
def build_rate_limiter(settings: CoreSettings) -> RateLimiter:
    ...
    return LocalFixedWindowRateLimiter(
        policies={
            ...
            "async_submit": RateLimitPolicyConfig(limit=settings.rate_limit_async_submit_per_minute),
            ...
            "control_plane": RateLimitPolicyConfig(limit=settings.rate_limit_control_plane_per_minute),
        }
    )
